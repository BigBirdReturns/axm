use crate::coords::{Coord, IR_SCHEMA_VERSION};
use crate::ir::{manifest_coordinate_schema, Fork, Node, Provenance, Relation, SourceInfo};
...
pub struct ProgramManifest {
    ...
    #[serde(default)]
    pub chunk_index: HashMap<String, serde_json::Value>,
    #[serde(default)]
...
impl Program {
    ...
    pub fn manifest(&self) -> ProgramManifest {
        ProgramManifest {
            ...
            coordinate_system: manifest_coordinate_schema(),
            counts: HashMap::from([
                ("nodes".into(), self.nodes.len()),
                ("relations".into(), self.relations.len()),
                ("forks".into(), self.forks.len()),
                ("provenance".into(), self.provenance.len()),
            ]),
    ...
    pub fn load_dir(path: impl AsRef<Path>) -> Result<Self, String> {
        ...
        let chunk_hashes = extract_chunk_hashes(&manifest);
    ...
    pub fn load_zip_bytes(bytes: &[u8]) -> Result<Self, String> {
        ...
        let manifest: ProgramManifest = serde_json::from_str(&manifest_str)
            .map_err(|e| format!("Invalid manifest: {}", e))?;
        let source: SourceInfo = serde_json::from_value(manifest.source.clone())
            .map_err(|e| format!("Invalid source: {}", e))?;
...
fn read_nodes(path: PathBuf) -> Result<HashMap<String, Node>, String> {
    let file = File::open(&path).map_err(|_| format!("Missing nodes.jsonl in {}", path.display()))?;
    ...
    for (idx, line) in reader.lines().enumerate() {
        ...
        let node: Node = serde_json::from_str(&line).map_err(|e| format!("Invalid node at line {}: {}", idx + 1, e))?;
...
fn read_relations(path: PathBuf) -> Result<Vec<Relation>, String> {
    ...
    for (idx, line) in reader.lines().enumerate() {
        ...
        let rel: Relation = serde_json::from_str(&line).map_err(|e| format!("Invalid relation at line {}: {}", idx + 1, e))?;
...
fn read_provenance(path: PathBuf) -> Result<HashMap<String, Provenance>, String> {
    ...
    for line in reader.lines() {
        ...
        let prov: Provenance = serde_json::from_str(&line).map_err(|e| format!("Invalid provenance: {}", e))?;
        provs.insert(prov.prov_id.clone(), prov);
    }
    Ok(provs)
}

fn read_nodes_from_zip<R: Read + std::io::Seek>(archive: &mut ZipArchive<R>, name: &str) -> Result<HashMap<String, Node>, String> {
    let mut file = archive.by_name(name).map_err(|_| format!("{} missing", name))?;
...
fn read_relations_from_zip<R: Read + std::io::Seek>(archive: &mut ZipArchive<R>, name: &str) -> Result<Vec<Relation>, String> {
    let mut rels = Vec::new();
...
fn read_forks_from_zip<R: Read + std::io::Seek>(archive: &mut ZipArchive<R>, name: &str) -> Result<Vec<Fork>, String> {
    let mut forks = Vec::new();
    if let Ok(mut file) = archive.by_name(name) {
        let mut data = String::new();
        file.read_to_string(&mut data)
...
fn read_provenance_from_zip<R: Read + std::io::Seek>(archive: &mut ZipArchive<R>, name: &str) -> Result<HashMap<String, Provenance>, String> {
    let mut provs = HashMap::new();
    if let Ok(mut file) = archive.by_name(name) {
        let mut data = String::new();
        file.read_to_string(&mut data)
...
fn extract_chunk_hashes(manifest: &ProgramManifest) -> HashMap<String, String> {
    let mut hashes = HashMap::new();
    for (chunk_id, info) in &manifest.chunk_index {
        if let Some(map) = info.as_object() {
            if let Some(hash) = map.get("content_hash").and_then(|h| h.as_str()) {
                hashes.insert(chunk_id.clone(), hash.to_string());
            }
        }
    }
    hashes
