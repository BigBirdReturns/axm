...
## v0.5.3
AXM v0.5.3 adds derivation execution as a first-class step. This release introduces a derivation engine with pluggable strategies and ships two built-in passes:
- temporal: creates TemporalAlignment edges per chunk by attaching nodes to detected reference time nodes
- confidence: materializes per-chunk confidence summary nodes and Derivation records (mean provenance confidence)

This release also aligns package metadata to 0.5.3.
